...
export class ObjectStorageService {
  ...
  async getObjectEntityUploadURL(clientId: string, fileName: string): Promise<{ uploadURL: string; objectPath: string }> {
    ...
    const fullPath = `${privateObjectDir}/clients/${clientId}/${objectId}_${sanitizedFileName}`;
    const { bucketName, objectName } = parseObjectPath(fullPath);
    ...
    return {
      uploadURL,
      objectPath: `/objects/clients/${clientId}/${objectId}_${sanitizedFileName}`,
    };
  }

  async getOfficeLogoUploadURL(officeId: string, fileName: string): Promise<{ uploadURL: string; objectPath: string }> {
    const privateObjectDir = this.getPrivateObjectDir();
    if (!privateObjectDir) {
      throw new Error(
        "PRIVATE_OBJECT_DIR not set. Create a bucket in 'Object Storage' " +
    ...
    const objectId = randomUUID();
    const sanitizedFileName = fileName.replace(/[^a-zA-Z0-9.-]/g, '_');
    const fullPath = `${privateObjectDir}/offices/${officeId}/${objectId}_${sanitizedFileName}`;
    const { bucketName, objectName } = parseObjectPath(fullPath);
    ...
    return {
      uploadURL,
      objectPath: `/objects/offices/${officeId}/${objectId}_${sanitizedFileName}`,
    };
  }
